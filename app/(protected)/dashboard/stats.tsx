    ...
    <div className='flex flex-wrap justify-between items-center w-full'>
      <div className='flex flex-wrap justify-between gap-4 w-full'>
        {stats.map((stat, index) => (
          <Card
            key={index}
            className='flex justify-center items-center p-5 rounded-md flex-1 w-full'
          >
            <div className='flex justify-between text-sm w-full flex-1 p-6'>
              <div
                className={`text-primary flex items-center justify-center p-4 rounded-full ${stat.iconBg}`}
              >
                <Icon
                  icon={`${stat.icon}`}
                  className={`h-6 ${stat.iconColor} w-6`}
                />
              </div>
